...
let world: World =
    Array2D.create 100 100 0 // 2 (fun x -> if x > 0 then 1 else 0)
        |> Array2D.insertAt (Life.Shapes.Methuselah.rPentomino) (40, 40) // |> Array2D.pad 2 0) (2, 2)
...
[<EntryPoint>]
let main argv: int =
    Life.recursiveSeq Array2D.TranslateMode.Zero 1000 world
        // |> Seq.length
        |> Seq.last |> Pretty.world true
        // |> Seq.takeEvery 20
